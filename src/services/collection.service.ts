...
export default class CollectionService extends BaseCRUDService<ICollection> {
  ...
  async createExternalNFTCollectionWithSign(request: ICollectionExternalNFTRequest): Promise<Result<ICollection>> {
    ...
    let collectionCreated: Result<ICollection> | null = await this.getByAddressAsync(request.data.address);

    if (!collectionCreated.success && !collectionCreated.data) {
      collectionCreated = await this.createAsync(<ICollection>{
        title: request.data.name,
        namelc: request.data.name.toLowerCase(),
        show: false,
        description: request.data.name,
        isCustom: true,
        owner: request.data.address.toLowerCase(),
        account: request.data.account.toLowerCase()
      });
    }
